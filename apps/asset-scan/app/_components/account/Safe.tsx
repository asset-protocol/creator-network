...
import { ArrowRight, ArrowRightLeft, CircleCheck } from "lucide-react";
import useToken from "antd/es/theme/useToken";

export class SafeSigner implements Required<AssetContractRunner> {
  isMulti: boolean = true;

  constructor(private safe: Safe, private chainId: number, private signer: Signer) {
  }
  ...
  return (
    <div>
      {loading ? <Skeleton.Input active /> :
        safeItems.length > 0 ? safeItems.map(a => (
          <div>
            <div className="flex">
              ...
              >
                {account?.address === a.safeAddress ?
                  <CircleCheck size={16} color={token.colorPrimary} /> : <ArrowRightLeft size={16} />}
              </Button>
            </div>
            {a.channels.length > 0 ?
              <div>
