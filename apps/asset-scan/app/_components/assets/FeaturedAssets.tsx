...
export async function FeaturedAssets({ className }: { className?: string }) {
  ...
  return <div className={clsx("flex", className)}>
    <div className="carousel carousel-vertical">
      {data.map((items, i) => {
        return <div key={i.toString()} className="carousel-item h-full w-full flex flex-col gap-4">
          {items.map(a => (<AssetItem key={a.id} asset={a} className="bg-base-100 rounded-md" />))}
        </div>
      })}
    </div>
...
function PopularAssetItem(props: AssetItemProps) {
  const { asset, className } = props;
  return <div className={clsx("grid grid-cols-2 w-full py-4 gap-4", className)}>
    ...
    <div className="flex-1 flex flex-col">
      ...
      <Link href={`/${asset.hub}/assets/${asset.assetId}`} className="text-base font-semibold mt-4 line-clamp-2">{asset.name}</Link>
...
export function AssetItem(props: AssetItemProps) {
  const { asset, className } = props;
  return <div className={clsx("grid grid-cols-2 w-full p-4 gap-4", className)}>
    <Link href={`/${asset.hub}/assets/${asset.assetId}`}>
      <Image
        src={replaceUri(asset.image)!}
        alt={asset.name}
    ...
    <div className="flex-1">
      ...
      <Link href={`/${asset.hub}/assets/${asset.assetId}`} className="text-xl font-bold mt-4 line-clamp-2">{asset.name}</Link>
      ...
      {props.descriptionDisabled !== true && <div className="text-gray-400 line-clamp-2 my-2 text-sm">{asset.description}</div>}
    </div>
